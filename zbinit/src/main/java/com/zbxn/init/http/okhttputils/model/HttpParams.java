package com.zbxn.init.http.okhttputils.model;
...
/**
 * ================================================ 作 者：廖子尧 版 本：1.0
 * 创建日期：2015/10/9 描 述：请求参数的包装类，支持一个key对应多个值 修订历史：
 * ================================================
 */
public class HttpParams implements Serializable {
	...
	public String toString() {
		...
		for (ConcurrentHashMap.Entry<String, List<String>> entry : urlParamsMap
				.entrySet()) {
			if (result.length() > 0)
				result.append("&");
			result.append(entry.getKey()).append("=").append(entry.getValue());
		}
		for (ConcurrentHashMap.Entry<String, List<FileWrapper>> entry : fileParamsMap
				.entrySet()) {
			if (result.length() > 0)
				result.append("&");
			result.append(entry.getKey()).append("=").append(entry.getValue());
