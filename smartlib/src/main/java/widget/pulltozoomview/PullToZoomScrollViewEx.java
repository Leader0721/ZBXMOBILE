...
public class PullToZoomScrollViewEx extends PullToZoomBase<ScrollView> {
        ...
        ((InternalScrollView) mRootView).setOnScrollViewChangedListener(new OnScrollViewChangedListener() {
            ...
            public void onInternalScrollChanged(int left, int top, int oldLeft, int oldTop) {
                if (isPullToZoomEnabled() && isParallax()) {
                	Logger.t(TAG).d("onScrollChanged --> getScrollY() = " + mRootView.getScrollY());
                    float f = mHeaderHeight - mHeaderContainer.getBottom() + mRootView.getScrollY();
                    Logger.t(TAG).d("onScrollChanged --> f = " + f);
                    ...
                    if(mOnScrollListener != null){
                    	float ratio = (mHeaderHeight - mRootView.getScrollY())/(float)mHeaderHeight;
                    	mOnScrollListener.onScroll(mRootView.getScrollY(), ratio);
                    }
                }
            }
        });
    ...
    protected void pullHeaderToZoom(int newScrollValue) {
    	Logger.t(TAG).d("pullHeaderToZoom --> newScrollValue = " + newScrollValue);
    	Logger.t(TAG).d("pullHeaderToZoom --> mHeaderHeight = " + mHeaderHeight);
    ...
    protected ScrollView createRootView(Context context, AttributeSet attrs) {
        ...
        scrollView.setId(R.id.pulltozoomview_scrollview);
        return scrollView;
    }

    @Override
    public void smoothScrollToTop() {
    	Logger.t(TAG).d("smoothScrollToTop --> ");
    ...
    public void handleStyledAttributes(TypedArray a) {
        ...
        //防止因为容器内的控件改变，造成ScrollView发生自动滚动
        mRootContainer.setFocusable(true);
        mRootContainer.setFocusableInTouchMode(true);
        ...
        public void run() {
            if (mZoomView != null) {
                ...
                if ((!mIsFinished) && (mScale > 1.0D)) {
                    ...
                    Logger.t(TAG).d("ScalingRunnable --> f2 = " + f2);
                    if (f2 > 1.0F) {
