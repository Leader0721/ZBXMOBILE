package widget.slidebottompanel;
...
public class SlideBottomPanel extends FrameLayout {
	...
	private static final int DEFAULT_TITLE_HEIGHT_NO_DISPLAY = 50;
	...
	public SlideBottomPanel(Context context, AttributeSet attrs,
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.SlideBottomPanel, defStyleAttr, 0);

		mBackgroundId = a.getResourceId(
				R.styleable.SlideBottomPanel_sbp_background_layout,
				DEFAULT_BACKGROUND_ID);
		mPanelHeight = a.getDimension(
				R.styleable.SlideBottomPanel_sbp_panel_height,
				dp2px(DEFAULT_PANEL_HEIGHT));
		mBoundary = a.getBoolean(R.styleable.SlideBottomPanel_sbp_boundary,
				DEFAULT_BOUNDARY);
		MAX_CLICK_DISTANCE = mTitleHeightNoDisplay = a.getDimension(
				R.styleable.SlideBottomPanel_sbp_title_height_no_display,
				dp2px(DEFAULT_TITLE_HEIGHT_NO_DISPLAY));
		mMoveDistanceToTrigger = a.getDimension(
				R.styleable.SlideBottomPanel_sbp_move_distance_trigger,
				dp2px(DEFAULT_MOVE_DISTANCE_TO_TRIGGER));
		mAnimationDuration = a.getInt(
				R.styleable.SlideBottomPanel_sbp_animation_duration,
				DEFAULT_ANIMATION_DURATION);
		mHidePanelTitle = a.getBoolean(
				R.styleable.SlideBottomPanel_sbp_hide_panel_title,
				DEFAULT_HIDE_PANEL_TITLE);
		mIsFade = a.getBoolean(R.styleable.SlideBottomPanel_sbp_fade,
				DEFAULT_FADE);
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		for (int i = 0; i < mChildCount; i++) {
			...
			if (childView.getTag() == null
					|| Integer.valueOf(childView.getTag().toString()) != TAG_BACKGROUND) {
				childView.layout(0, t, childView.getMeasuredWidth(),
						childView.getMeasuredHeight() + t);
				childView.setTag(TAG_PANEL);
				// if (childView instanceof ViewGroup) {
				// ((ViewGroup)childView).setClipChildren(false);
				// }
			} else if (Integer.parseInt(childView.getTag() + "") == TAG_BACKGROUND) {
	...
	private void initBackgroundView() {
		if (mBackgroundId != -1) {
			...
			mDarkFrameLayout.addView(LayoutInflater.from(mContext).inflate(
					mBackgroundId, null));
	...
	private void handleActionUp(MotionEvent event) {
		...
		computeVelocity();
		if (!isPanelShowing
				&& ((event.getY() - firstDownY) < 0 && (Math.abs(event.getY()
						- firstDownY) > mMoveDistanceToTrigger))
				|| (yVelocity < 0 && Math.abs(yVelocity) > Math.abs(xVelocity) && Math
						.abs(yVelocity) > mMinVelocity)) {
			displayPanel();
		...
		if (isPanelShowing) {
			...
			if (currentY < (mMeasureHeight - mPanelHeight)
				...
				ObjectAnimator
						.ofFloat(mPanel, "y", currentY,
								mMeasureHeight - mPanelHeight)
						.setDuration(mAnimationDuration).start();
			} else if (currentY > mMeasureHeight - mPanelHeight
	...
	private void handleActionMove(MotionEvent event) {
		...
		if (isPanelShowing
				&& supportScrollInView((int) (firstDownY - event.getY()))) {
			return;
		}
		computeVelocity();
		...
		if (isDragging) {
			...
			if (mDarkFrameLayout != null && mIsFade) {
				...
				if (currentY > mMeasureHeight - mPanelHeight
					...
					mDarkFrameLayout
							.fade((int) ((1 - currentY
									/ (mMeasureHeight - mTitleHeightNoDisplay)) * DarkFrameLayout.MAX_ALPHA));
				}
			}
			if (!mBoundary) {
				...
				if (touchingViewY + deltaY <= mMeasureHeight - mPanelHeight) {
					touchingView.offsetTopAndBottom((int) (mMeasureHeight
							- mPanelHeight - touchingViewY));
				} else if (touchingViewY + deltaY >= mMeasureHeight
						- mTitleHeightNoDisplay) {
					touchingView.offsetTopAndBottom((int) (mMeasureHeight
							- mTitleHeightNoDisplay - touchingViewY));
				} else {
					touchingView.offsetTopAndBottom((int) deltaY);
				}
			}
	...
	private void hidePanel() {
		...
		ValueAnimator animator = ValueAnimator.ofFloat(ViewHelper.getY(mPanel),
				mMeasureHeight - mTitleHeightNoDisplay);
		animator.setInterpolator(mCloseAnimationInterpolator);
		animator.setTarget(mPanel);
		animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			@Override
			public void onAnimationUpdate(ValueAnimator animation) {
				float value = Float.valueOf(animation.getAnimatedValue()
						.toString());
				ViewHelper.setY(mPanel, value);
				if (mDarkFrameLayout != null && mIsFade && value < t) {
					mDarkFrameLayout
							.fade((int) ((1 - value / t) * DarkFrameLayout.MAX_ALPHA));
				}
			}
		});
	...
	public void displayPanel() {
		...
		final View mPanel = findViewWithTag(TAG_PANEL);
		ValueAnimator animator = ValueAnimator.ofFloat(ViewHelper.getY(mPanel),
				mMeasureHeight - mPanelHeight).setDuration(mAnimationDuration);
		animator.setTarget(mPanel);
	...
	private boolean supportScrollInView(int direction) {
		...
		if (view instanceof ViewGroup) {
			View childView = findTopChildUnder((ViewGroup) view, firstDownX,
					firstDownY);
			if (childView == null) {
				return false;
			}
			if (childView instanceof AbsListView) {
				...
				View grandchildView = findTopChildUnder((ViewGroup) childView,
						firstDownX, firstDownY);
				...
				if (grandchildView instanceof ViewGroup) {
					if (grandchildView instanceof AbsListView) {
						...
						if (Build.VERSION.SDK_INT >= 19) {
							...
							return absListViewCanScrollList(absListView,
									direction);
						}
						...
						if (Build.VERSION.SDK_INT >= 14) {
							...
							return scrollViewCanScrollVertically(scrollView,
									direction);
						}
					}
				}
	 ...
	 *            down.
	 * @return true if the scrollView can be scrolled in the specified
	 *         direction, false otherwise
	 */
	private boolean scrollViewCanScrollVertically(ScrollView scrollView,
			int direction) {
		final int offset = Math.max(0, scrollView.getScrollY());
		final int range = computeVerticalScrollRange(scrollView)
	...
	private int computeVerticalScrollRange(ScrollView scrollView) {
		...
		final int contentHeight = scrollView.getHeight()
	 ...
	 *         false otherwise
	 */
	private boolean absListViewCanScrollList(AbsListView absListView,
			int direction) {
		...
		if (direction > 0) {// can scroll down
			final int lastBottom = absListView.getChildAt(childCount - 1)
					.getBottom();
			final int lastPosition = firstPosition + childCount;
			return lastPosition < absListView.getCount()
					|| lastBottom > absListView.getHeight()
