...
import com.zbar.lib.config.Config;
...
final class CameraConfigurationManager {
	...
	void initFromCameraParameters(Camera camera) {
		...
		Logger.i("Screen resolution: " + screenResolution);
		...
		Logger.i("Camera resolution: " + cameraResolution);
	}
	...
	void setDesiredCameraParameters(Camera camera, boolean safeMode) {
		...
		if (parameters == null) {
			Logger.w("Device error: no camera parameters are available. Proceeding without configuration.");
			return;
		}

		Logger.i("Initial camera parameters: " + parameters.flatten());

		if (safeMode) {
			Logger.w("In camera config safe mode -- most settings will not be honored");
		}
		...
		if (afterSize != null
			...
			Logger.w("Camera said it supported preview size "
	...
	private Point findBestPreviewSizeValue(Camera.Parameters parameters,
		...
		if (rawSupportedSizes == null) {
			Logger.w("Device returned no supported preview sizes; using default");
		...
		Logger.i("Supported preview sizes: " + previewSizesString);
		...
		while (it.hasNext()) {
			...
			if (maybeFlippedWidth == screenResolution.x
				...
				Logger.i( "Found preview size exactly matching screen size: "
						+ exactPoint);
				return exactPoint;
			}
		}
		...
		if (!supportedPreviewSizes.isEmpty()) {
			...
			Logger.i("Using largest suitable preview size: " + largestSize);
			return largestSize;
		}
		...
		Logger.i("No suitable preview sizes, using default: " + defaultSize);
	...
	private static String findSettableValue(Collection<String> supportedValues,
		...
		Logger.i("Supported values: " + supportedValues);
		String result = null;
		...
		Logger.i("Settable value: " + result);
		return result;
	}
