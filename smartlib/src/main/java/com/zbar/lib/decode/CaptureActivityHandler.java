...
import com.zbar.lib.view.ViewfinderResultPointCallback;
...
public final class CaptureActivityHandler extends Handler {
	...
	public void handleMessage(Message message) {
		if (message.what == R.id.zbar_restart_preview) {// 准备进行下一次扫描
			Logger.d("Got restart preview message");
			restartPreviewAndDecode();
		} else if (message.what == R.id.zbar_decode_succeeded) {
			...
			cameraManager.requestPreviewFrame(decodeThread.getHandler(),
					R.id.zbar_decode);
		} else if (message.what == R.id.zbar_return_scan_result) {
			Logger.d("Got return scan result message");
			activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
			activity.finish();
		} else if (message.what == R.id.zbar_launch_product_query) {
			Logger.d("Got product query message");
			...
			if (resolveInfo != null && resolveInfo.activityInfo != null) {
				...
				Logger.d("Using browser in package " + browserPackageName);
			}
			...
			try {
				...
				Logger.w("Can't find anything to handle VIEW of URI " + url);
			}
		}
	}
	...
	public void quitSynchronously() {
		...
		Message quit = Message
				.obtain(decodeThread.getHandler(), R.id.zbar_quit);
		quit.sendToTarget();
