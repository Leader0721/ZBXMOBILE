...
final class DecodeHandler extends Handler {
	...
	public void handleMessage(Message message) {
		...
		else if(message.what == R.id.zbar_quit){
			running = false;
	...
	private void decode(byte[] data, int width, int height) {
		...
		if (rawResult != null) {
			...
			Logger.d("Found barcode in " + (end - start) + " ms");
			if (handler != null) {
				Message message = Message.obtain(handler,
						R.id.zbar_decode_succeeded, rawResult);
			...
			if (handler != null) {
				Message message = Message.obtain(handler, R.id.zbar_decode_failed);
				message.sendToTarget();
			}
		}
	}
